...
read_continuous_data <- function(parameter, site_no, start, end){
  data_url <- paste0("https://nwis.waterservices.usgs.gov/nwis/iv/",
                     ...
                     glue("startDT={start}T00:00:00.000-08:00&"),
...
```{r}
# df of all dates in range to fill gaps
full_dates <- seq.Date(from = ymd(start), 
                       to = ymd(end), 
                       by = "day") %>%   tibble("date" = .) 

daily_data <- gage_height_df %>% 
  group_by(date) %>% 
  summarise(avg_gage_height_ft = mean(gage_height_ft)) %>% 
  ungroup() %>% 
  full_join(discharge_df %>% 
              group_by(date) %>% 
              summarize(avg_discharge_cfs = mean(discharge_cfs)) %>% 
              ungroup(), 
            by = "date") %>% 
  full_join(full_dates, by = "date") 
  # interpolate missing discharge data 
...
gage_height_ts <- ts(gage_height_df$gage_height_ft)
