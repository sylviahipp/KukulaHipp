...
1. Elwha River daily sediment loads and concentrations during and after dam removal
2. Elwha River water quality at the estuary
3. Invertebrate abundance and density at multiple sites in the estuary
4. Chinook and Coho salmon diets, sampled at estuary sites

Additionally, we obtained continuous streamflow data along the Elwha River from USGS's Water Data for the Nation. These data comprised of daily discharge and gage height recorded at USGS monitoring station #12045500 (Elwha River at McDonald Bridge near Port Angeles, WA). For a discussion on data availability from this and other monitoring stations along the Elwha River, see **Section III.A** below. 
...
Water quality data were obtained from USGS's Water Data for the Nation data repository. Historical water data for three USGS monitoring sites along the Elwha River are available. **Table 1** outlines the availability of river discharge and gage height data at each monitoring site. 
...
Our goal is to analyze Elwha River streamflow before, during, and after the dam removal, which occurred over the period 2011–2014. Based on data availability, we focused on discharge data obtained from USGS site #12045500 and gage height data from USGS sites #12044900 and #12045500. We pulled continuous data over the period 2008–2017 (as available) to capture a 10-year window surrounding the dam removal. 
...
```{r echo=FALSE, fig.align='center', fig.height=4, fig.cap="Elwha River Average Daily Discharge}
...
```{r echo=FAlSE, fig.align='center', fig.height=4, fig.cap="Elwha River Average Daily Gage Height}
# Figure: Gage Height Data Sites 12045500 and 12044900
gage_height_explore_plot <- ggplot() + 
  geom_rect(aes(xmin = start_removal, xmax = complete_removal, 
                ymin = 0, ymax = 600), alpha = 0.4, fill = "grey") + 
  geom_line(data = gage_height_df, 
            aes(x = date, y = avg_gage_height_ft, color = site_no)) + 
  annotate(geom = "text", x = ymd("2013-03-01"), y = 500, size = 2.5,
           label = "Dam Removal Period\n(Sept 2011-Aug 2014)") +
  scale_x_date(expand=c(0,0)) +
...
gage_height_explore_plot

```

**Figures 4** and **5** show daily average discharge (in cubic feet per second) and gage height (in feet), respectively, measured at the USGS monitoring sites with available data. As is apparent in **Figure 5**, gage height at USGS Site #12044900 (located upriver from the Elwha Dam) experienced an immediate spike from 10 ft to nearly 600 ft immediately at the start of the Elwha Dam removal in September 2011. Because the cause and characterization of this spike is unknown, we chose to focus only on USGS Site #12045500 for our analysis of discharge and gage height surrounding the Elwha River dam removals. 
