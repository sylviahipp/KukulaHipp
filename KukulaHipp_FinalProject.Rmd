...
kable(head(estuary_wq_raw %>% 
             select(Field.Activity.Number:Phosphate.concentration), 5), 
      caption = "Daily Sediment Concentration and Load Raw Data") %>% 
  row_spec(0, bold = TRUE) %>%
  kable_classic(full_width = FALSE)
```
...
Data concerning the dietary intake of two salmon species, Chinook and Coho, was obtained from USGS ScienceBAse reposity (https://doi.org/10.5066/F75B00N4), published in support of "Ecological parameters in the Elwha River estuary before and during dam removal" by Foley et al (see **Appendix**). In this dataset, the composition of Chinook and Coho salmon diets are made available from stomach content analysis. Fish were collected for sampling throughout 2006-2007 ("Before") and 2013-2014 ("During").For Chinook, 93 and 164 fish were sampled from before and during removal, respectively. For Coho, 23 and 112 fish were sampled from before and during removal, respectively. 193 unique taxa were screened for. **Table 4** shows a glimpse of the dataset, with the columns being truncated for clarity. In the complete dataset, each taxa screened for is assigned a unique column.
...
kable(head(salmonid_diet_raw %>% 
             select(Date:Amphipoda...unknown), 5), 
      caption = "Daily Sediment Concentration and Load Raw Data") %>% 
  row_spec(0, bold = TRUE) %>%
  kable_classic(full_width = FALSE)
```
...
To better understand the individual trends and suitability of this data for analysis, each parameter was plotted against time (**Figure #** - other plots not shown - for final plot of parameters see **Figure #**).
...
To assess the different taxa that comprised the diet, a dataset that contained the sum of each taxa count in each year was created. Some taxa are broken into distinct groups, such as Acari, which have entries for both larva and terrestrial (unknown). In the processed dataset, taxa from unique locations or growth stages were combined. In this example, Acari larva and terrestrial (unknown) were summed into a final "Acari" group. For each year, the top 10 taxa observed taxa were then plotted on a histogram (**Figures # and #**) to assess general trends.
...
Upon initial investigation, it is clear that the diet of Chinook salmon has diversified by 2014.
...
In Coho, similar trends in diversification are seen, however they begin earlier.
...
```{r echo = FALSE, fig.cap = "Top taxa for Chinook salmon before and after dam removal"}
...
```{r echo = FALSE, fig.cap = "Top taxa for Coho salmon before and after dam removal"}
