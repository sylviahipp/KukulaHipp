...
```{r}
summary(gage_height_df)
summary(discharge_raw)
...
gage_height_df <- gage_height_raw %>% 
  ...
  group_by(date) %>% 
  summarize(avg_gage_height_ft = mean(gage_height_ft, na.rm = TRUE)) %>% 
  ungroup()
...
discharge_df <- discharge_raw %>% 
  ...
  group_by(date) %>% 
  summarize(avg_discharge_cfs = mean(discharge_cfs, na.rm = TRUE)) %>% 
  ungroup()
