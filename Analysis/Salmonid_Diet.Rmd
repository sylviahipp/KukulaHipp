...
diet_totals:  total counts of individual taxa observed per year and before/after dam removal for all salmon sampled
chinook_diet:  diet_totals for Chinook only
coho_diet:  diet_totals for Coho only
...
```{r}
# Create a data frame that contains the number of unique taxa observed in salmon 
# diets in each year
...
test_plot <- ggplot(test, aes(x = taxa, y = value, fill = name)) +
  geom_bar(stat = "identity", position = "dodge")+
    theme(axis.text.x = element_text(angle = 45, hjust = 1))


# top
chinook_taxa_after <- chinook_diet %>% 
  select(fish:taxa, after_count) %>% 
