...
chinook_diet:  salmonid_diet_processed for Chinook only
...
## Creating individual dataframes for Chinook and Coho
...
# chinook_top_taxa_before <- 
#   # Create a combined dataset with the top 10 taxa from 2006 and top 10 taxa from 2007
#   full_join(chinook_taxa_2006 %>% head(20),
#             chinook_taxa_2007 %>% head(20),
#             by = c("fish", "taxa")) %>%
...
# chinook_top_taxa_after <-
#   # Create a combined dataset with the top 10 taxa from 2013 and top 10 taxa from 2014
#   full_join(chinook_taxa_2013 %>% head(20),
#             chinook_taxa_2014 %>% head(20),
#             by = c("fish", "taxa")) %>% 
#   # Join original data to fill missing values
#   inner_join(chinook_diet %>%
...
# chinook_top_taxa_overlap <- inner_join(chinook_top_taxa_before %>% 
...
# view(chinook_top_taxa_overlap)
# 
# 
# 
# # Plotting total before and after counts for the taxa that were present in before and after 
# chinook_top_taxa_before_overlap_plot <- ggplot(data = chinook_top_taxa_overlap,
#                                        aes(x = taxa,
#                                            y = value,
#                                            fill = name)) +
...
# chinook_top_taxa_after_overlap_plot <- ggplot(data = chinook_top_taxa_overlap %>% filter(name == c("value_2013", "value_2014")),
#                                        aes(x = taxa,
#                                            y = value,
#                                            fill = name)) +
#   geom_bar(stat = "identity", position = "stack") +
...
# Plotting the changes in Chinook taxa
...
# Before
chinook_top_before_taxa <- salmonid_diet_processed %>%
  filter(fish == "Chinook") %>%
...
chinook_top_before_taxa_plot <- ggplot(chinook_top_before_taxa, aes(x = taxa, y = value, fill = as.factor(name))) +
  geom_bar(stat = "identity", position = "dodge") +
    theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
...
dam_state_legend <- get_legend(ggplot(chinook_top_after_taxa, aes(x = taxa, y = value, fill = name)) +
  geom_bar(stat = "identity", position = "dodge") +
    labs(fill = NULL) +
